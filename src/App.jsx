...
// --- Widgets ---
// Each widget uses registerRef for position saving
...
function PollWidget({ onRemove, onRename, position, registerRef }) {
  ...
  useEffect(()=>setVotes(v=>{if(v.length===options.length)return v;if(v.length<options.length)return[...v,...Array(options.length-v.length).fill(0)]; return v.slice(0,options.length)}),[options]);
  const vote=i=>setVotes(v=>v.map((val,idx)=>idx===i?val+1:val));
  const addOption=()=>{const t=newOption.trim();if(!t)return; setOptions(o=>[...o,t]); setNewOption('')};
  const removeOption=i=>{setOptions(o=>o.filter((_,idx)=>idx!==i)); setVotes(v=>v.filter((_,idx)=>idx!==i))};
  const updateOption=(i,v)=>setOptions(o=>o.map((opt,idx)=>idx===i?v:opt));
  return <div ref={ref}><WidgetWrapper title="Poll" onRemove={onRemove} onRename={onRename}><div style={{ display:'flex', flexDirection:'column', gap:'8px'}}>{options.map((opt,i)=><div key={i} style={{ display:'flex', alignItems:'center', gap:'8px'}}><input value={opt} onChange={e=>updateOption(i,e.target.value)} style={{flex:1}}/><button onClick={()=>vote(i)}>Vote</button><div style={{ minWidth:'60px', textAlign:'right'}}>{votes[i]} votes</div><button onClick={()=>removeOption(i)} style={{ marginLeft:'6px'}}>Remove</button></div>)}<div style={{ display:'flex', gap:'8px', marginTop:'6px'}}><input placeholder="New option" value={newOption} onChange={e=>setNewOption(e.target.value)} style={{flex:1}}/><button onClick={addOption}>Add</button></div></div></WidgetWrapper></div>;
}
...
// Main ClassroomScreen component
export default function ClassroomScreen() {
  ...
  return (
    <div style={{height:'100vh', width:'100vw', position:'relative', overflow:'hidden'}}>
      ...
      {/* Widgets */}
      {widgets.map(w=>{
        const commonProps={key:w.id,onRemove:()=>removeWidget(w.id),onRename:t=>renameWidget(w.id,t),position:w.position,registerRef:(getPos)=>widgetRefs.current[w.id]=getPos};
        switch(w.type){
        ...
        <div style={{
          ...
          {/* Top row with minimize and heading */}
          ...
          {/* Widget buttons, only when bar is open */}
          {!barMinimized && (
            <div style={{ display:'flex', justifyContent:'center', gap:'12px', flexWrap:'wrap', width:'100%' }}>
              {['stoplight','clock','timer','poll','dice','namepicker','image','conversion'].map(t=>(
                <button key={t} style={glassButtonStyle} onClick={()=>addWidget(t)}>
